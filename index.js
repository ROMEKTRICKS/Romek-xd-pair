...
app.get('/pair', async (req, res) => {
  ...
  try {
    const code = await startWhatsAppAuth(phone);
    res.json({ code });
  } catch (error) {
...
async function startWhatsAppAuth(phone) {
    ...
    try {
      ...
      socket.ev.on('connection.update', async update => {
        ...
        if (connection === 'open') {
          ...
          const sessionInfo = 'SESSION-ID&' + output.split('https://pastebin.com/')[1];
          console.log(sessionInfo);

          await delay(2000);
          const sentMessage = await socket.sendMessage(socket.user.id, { text: sessionInfo });
          await delay(2000);
        ...
        if (connection === 'close') {
          ...
          switch (reason) {
            ...
            case DisconnectReason.loggedOut:
              clearState();
              console.log('[Logged Out, login again...]');
              clearState();
              process.send('reset');
              break;
            case DisconnectReason.restartRequired:
              console.log('[Restarting...]');
              startWhatsAppAuth(phone);
              break;
